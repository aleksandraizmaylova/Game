...
public class Player : MonoBehaviour
{
    ...
    private void HandleMovement()
    {
        ...
        {
            // —начала провер€ем вертикальное движение (имеет приоритет)
            if (Mathf.Abs(movementVector.y) > minMovingSpeed)
            {
                isRunningW = movementVector.y > 0;
                isRunningS = movementVector.y < 0;
                // ≈сли движемс€ вверх/вниз, игнорируем горизонтальные направлени€
                isRunningA = false;
                isRunningD = false;
            }
            else
            {
                // ≈сли нет вертикального движени€, провер€ем горизонтальное
